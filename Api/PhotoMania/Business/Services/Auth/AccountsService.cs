...
namespace PhotoMania.Business.Services.Auth
{
    public class AccountsService: IAccountsService
    {
        ...
        public AccountsService(ITokenService tokenService, IUnitOfWork uow, IMD5Service md5Service)
        {
        ...
        public async Task<string> GetAccessToken(string login, string password)
        {
            ...
            if (!account.Password.Equals(md5Service.Hash(password)))
            {
            ...
            return tokenService.CreateToken(account.Login, role);
        }
    }
}
